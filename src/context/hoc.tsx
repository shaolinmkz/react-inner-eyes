...
const withInnerEyes = (Comp: FC): Function => {
  return (props: ComponentProps<any>) => {
    ...
    if(typeof name === 'string' && typeof value === 'function' && !isExist(name, state)) {
      ...
      console.error(`${name} is not a string, The first argument should be a string`);
    } else {
      console.error(`${value} is not a function, The second argument should be a function`);
    }
  };
